...
Some things you need to be sure of is that the wildcard DNS record in your DNS provider is pointing to the server IP where Coolify instance lives and that the wildcard domain you're entering is being served by Coolify.
...
Head over the the Sources page so we can create one.

![navigation to the sources page](../../../public/images/blog/pull-request-previews/coolify-menu-sources.png)

Click on "+ Add" and you'll be taken to different screen and you'll enter some details about the GitHub you're about to create.
...
The next screen is where Coolify needs to register the GitHub App. Before we do that, we're going to change the "Webhook Endpoint" so we don't expose our remote server IP to the world. I'm using the domain where my Coolify instance is lives.
...
You should be taken to a Github page in the browser where it will ask you to login. I have 2FA set up so this screen may be different for you. Just enter your details and you should be good to go.
...
I had to adjust the Branch to "master" because the dropdown list is ordered alphabetically. If your site is static, be sure to toggle that on. You can change all of this in the configuration page if you mess up. Click "Continue".

![configure github repository](../../../public/images/blog/pull-request-previews/coolify-load-repository.png)

This next section should be familiar if you had followed my other tutorial when I set up self-hosting for this website.
...
These next steps can be ignore if you don't want to set resource limits for your application. In my case, since this application is a demo, I don't want it to hog resources in case there are a lot of requests coming from it. Click on "Resource Limits" if you want this feature.
...
Talk about awesomeness! Its integrated in our GitHub Workflow now.
...
So now that we have changed our configuration, we need to redeploy our Preview Deployment build so that we can see changes to the URL.
...
You'vee learned how to use Coolify to create Preview Deployments for your Pull Requests.
